...
module.exports = class CreateFunctionUpdate001 extends Step {
  constructor(baseName, baseVersion) {
    ...
    this.sql = `
    CREATE OR REPLACE FUNCTION ${this.name}(chelate JSONB, _owner OWNER_ID) RETURNS JSONB
      AS $$
        ...
        -- needs to detect tk only changes
          ...
          if ${this.kind}_${this.version}.changed_key(chelate) then
            ...
            if old_chelate is NULL then
              ...
              return format('{"status":"404", "msg":"Not Found", "extra":"update C","pk":"%s","sk":"%s","key":"%s"}',lower(chelate ->> 'pk'), (chelate ->> 'sk'), _owner.id)::JSONB;
            end if;
            ...
            if not(FOUND) then
               ...
               return format('{"status":"404", "msg":"Not Found", "extra":"D", "key":"%s", "pk":"%s", "sk":"%s"}',_owner.id, chelate ->> 'pk', chelate ->> 'sk')::JSONB;
  ...
  grant EXECUTE on FUNCTION ${this.name}(JSONB,OWNER_ID) to api_user;
  */
    `;
