...
module.exports = class FunctionDeleteTest extends Step {
  constructor(kind, baseVersion) {
    ...
    this.sql = `BEGIN;
      ...
      insert into ${this.kind}_${this.version}.one
        (pk,sk,tk,form,owner)
        values
        (format('%s#%s',${this.pkName}, ${this.username}), 
         ${this.recType}, 
         '${this.key1}', 
         ${this.form}, 
         '${this.key1}' ) ;
      ...
      SELECT has_function(
          ...
          'DB Function Delete (jsonb,owner) exists'
      );
      ...
      SELECT is (
        ...
        ${this.kind}_${this.version}.delete( 
          ${this.criteriaNF}::JSONB, 
          '("${this.key1}")'::OWNER_ID )::JSONB,
