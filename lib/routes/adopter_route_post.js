...
module.exports = {
  ...
  handler: async function (req) {
    ...
    let payload = req.payload || false;
    let form = payload.form || false;
    let user_key = payload.user_key || uuidv4() || false;
    ...
    try {
      ...
      if (rollback) {
        // rollback post ... a test
        // console.log('  /adopter POST rollback');
      ...
      result['error'] = err;
      console.error('/adoptor POST (TEXT, JSON) err', err);
    } finally {
  ...
  options: {
        ...
        validate: {
          payload: Joi.object({
            ...
            user_key: Joi.string().min(1).max(320).required(), 
          }),
          headers: Joi.object({
