...
module.exports = {
  ...
  handler: async function (req) {
    ...
    const apiTest = req.headers.apitest || false;
    const apiDebug = req.headers.apidebug || false;
    const apiTimeout = req.headers.apitimeout || 1800;
    // const test =     options.test || false;
    // const apiDebug = options.debug || false;
    
    /* 
    let token_timeout = options.token_timeout || Consts.default_timeout();
    if (token_timeout > Consts.default_timeout()) {
      // [Maximumn token timeout should never exceed Consts.default_timeout()]
      token_timeout = Consts.default_timeout();
    }
    */
    ...
    try {  
      ...
      const res = await client.query(
        {
          text: 'select * from api_0_0_1.signin($1::TEXT,$2::JSON,$3::integer)',
  ...
  options: {
        cors: {
          ...
          exposedHeaders: ['Accept', 'ApiDebug', 'ApiTimeout', 'ApiTest'],
        },
